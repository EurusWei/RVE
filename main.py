...
def Brutal_select_window_size(scores, scores_hat, wind_size_choice, n_wid, n_hei, diag):
    """
    This is the main function to calculate statistics for each local window of each candidate window size.
    Input:
        scores: the raw score vectors
        scores_hat: centered score vectors
        wind_size_choice: a list of candidate window sizes
        n_wid, n_hei: width and height of the input micrograph (only the part for each pixel 
        has a computed score vector)
        diag: boolean variable to indicate which A matrix to use (if True, uses the raw covariance 
        matrix, o.w. uses its diagonal matrix)
    Output: 
        window_t2_1: a list which has the same length as wind_size_choice, each element is the list 
        of statistics for each local subregion generated by the corresponding window size.
    """
    ...
    for i in range(len(wind_size_choice)):
        ...
        for i in range((n_hei-2*w)*(n_wid-2*w)):
...
if __name__ == '__main__':
    ...
    #plot window size versus statistics 
    #plt.plot(wid_ls[1:], t2_ls[1:])
